...
// for listing based on whether it filename or full path matches
...
func (w *Watcher) listRecursive(name string) (map[string]os.FileInfo, error) {
	...
	return fileList, filepath.WalkDir(name, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
...
func (w *Watcher) retrieveFileList() (map[string]os.FileInfo, map[string]os.FileInfo) {
	w.mu.Lock()
	defer w.mu.Unlock()
	currFiles := make(map[string]os.FileInfo, len(w.files))
	for k, v := range w.files {
		currFiles[k] = v
	}
...
func (w *Watcher) Start(d time.Duration) error {
	...
	for {
		...
		currFiles, newFiles := w.retrieveFileList()
		...
		// Look for events.
		go func() {
			w.pollEvents(currFiles, newFiles, evt, cancel)
			done <- struct{}{}
		}()
