...
func Parse(input string, removeCurlyBraces ...bool) (ExprNode, error) {
	if len(removeCurlyBraces) > 0 {
		if removeCurlyBraces[0] {
...
func parseValue(s *TokenStream) (ExprNode, error) {
	...
	case TokenKindIdentifier:
		...
		case "null":
			return NewExprNodeLiteral(nil, token.SourcePos, token.SourceLen), nil
		}
