...
func (r *ReplaceDocx) Editable() *Docx {
	return &Docx{
		...
		images:            r.images,
		opt:               r.opt,
		placeholderParser: r.placeholderParser,
	}
}
...
func (d *Docx) Placeholders() (placeholders []string) {
	matches := d.placeholderParser.FindAllStringSubmatch(d.content, -1)
...
func (d *Docx) Parse(data map[string]any) (err error) {
	for key, val := range data {
		err = d.Replace(key, fmt.Sprint(val), -1)
		if err != nil {
			return
		}
	}
...
func (d *Docx) ReplaceAll(data map[string]any) (err error) {
	for key, val := range data {
		err = d.Replace(key, fmt.Sprint(val), -1)
		if err != nil {
			return
...
func ReadDocxFromFS(file string, fs fs.FS, opt Option) (*ReplaceDocx, error) {
	f, err := fs.Open(file)
	if err != nil {
		return nil, err
	}
	...
	return ReadDocxFromMemory(reader, size, opt)
}

func ReadDocxFromMemory(data io.ReaderAt, size int64, opt Option) (*ReplaceDocx, error) {
	reader, err := zip.NewReader(data, size)
	if err != nil {
		return nil, err
	}
	zipData := ZipInMemory{data: reader}
	return ReadDocx(zipData, opt)
}

func ReadDocxFile(path string, opt Option) (*ReplaceDocx, error) {
...
func ReadDocx(reader ZipData, opt Option) (*ReplaceDocx, error) {
	if opt.LeftDelim == "" {
		opt.LeftDelim = "{"
	}
	if opt.RightDelim == "" {
	...
	pattern := fmt.Sprintf(`\%s([^}]+)\%s`, opt.LeftDelim, opt.RightDelim)
	placeholderParser := regexp.MustCompile(pattern)
	...
	return &ReplaceDocx{zipReader: reader, content: content, links: links, headers: headers, footers: footers, images: images, opt: opt, placeholderParser: placeholderParser}, nil
}
