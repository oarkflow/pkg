...
type TableList struct {
	...
	VerticalHeaderPadding float64

	HorizontalHeaderPadding float64
	// VerticalContentPadding define the space between lines in content.
	VerticalContentPadding float64

	HorizontalContentPadding float64
...
func (s *TableListContent) ToTextProp(align consts.Align, top, left, right float64, extrapolate bool, verticalPadding float64) Text {
	textProp := Text{
		Family:          s.Family,
		Style:           s.Style,
		Size:            s.Size,
...
func (s *TableList) MakeValid(header []string, defaultFamily string) {
	...
	if s.HorizontalContentPadding < zeroValue {
		s.HorizontalContentPadding = zeroValue
	}
}
